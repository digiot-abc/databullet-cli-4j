            ...
            for (ForeignKey foreignKey : table.getForeignKeys()) {
                for (ColumnReference columnReference : foreignKey.getColumnReferences()) {
                    ...
                    if (dataSpecColumn.getType() == null) {
                        ...
                        dataSpecType.setName(ReferenceOptions.class.getAnnotation(JsonTypeName.class).value());
                        ReferenceOptions referenceOptions = new ReferenceOptions();
                        dataSpecType.setOptions(referenceOptions);
                        dataSpecColumn.setType(dataSpecType);
                    }

                    ReferenceOptions referenceOptions = (ReferenceOptions) dataSpecColumn.getType().getOptions();
                    referenceOptions.setReferencedTable(foreignKey.getReferencedTable().getName());
                    referenceOptions.setReferencedColumn(columnReference.getPrimaryKeyColumn().getName());
                }
            }
